...
bool timKiem(string s, GiangVien ten) {
    for (int i = 0; i < s.length(); i++)
        s[i] = toupper(s[i]);
    for (int i = 0; i < ten.ten.length(); i++)
        ten.ten[i] = toupper(ten.ten[i]);
    if ((int)ten.ten.find(s) == -1)
        return false;
    return true;
}
...
int main() {
    ...
    cin >> r;
    cin.ignore();
    string* tV = new string[r];
    for (int i = 0; i < r; i++) {
        getline(cin, tV[i]);
        cout << "DANH SACH GIANG VIEN THEO TU KHOA " << tV[i] << ":" << endl;
        for (int j = 0; j < n; j++) {
            if (timKiem(tV[i], gv[j]))

                cout << gv[j].maGV << " " << gv[j].ten << " " << layTat(gv[j].boMon) << endl;
